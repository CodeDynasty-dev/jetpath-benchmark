  ...
  try {
    ...
    const updateProgress = (current: number, total: number) => {
      ...
      process.stdout.write(
        `\r${CYAN}[${RESET}${"■".repeat(completeSize)}${
          " ".repeat(remainingSize)
        }${CYAN}]${RESET} ${percent}% (${current}/${total})`,
      );
    };
    ...
    await runConcurrentRequests(BENCHMARK_REQUESTS, fullUrl, (result) => {
      ...
      if (result.success) {
        ...
        results.statusCodes[statusKey] = (results.statusCodes[statusKey] || 0) +
          1;
      } else {
        results.failed++;
        if (result.error) {
    ...
    results.totalDuration = results.timeCompleted.getTime() -
      results.timeStarted.getTime();
    results.successRate = (results.successful / BENCHMARK_REQUESTS) * 100;

    if (results.responseTimes.length > 0) {
      results.avgResponseTime = results.responseTimes.reduce((sum, time) =>
        sum + time, 0) / results.responseTimes.length;
...
function calculateScore(
  ...
  const avgTimeScore =
    Math.min(otherResult.avgResponseTime, result.avgResponseTime) /
...
function visualizeResponseTimes(results: BenchmarkResult[]): string {
  ...
  buckets.forEach((bucket, i) => {
    ...
    output += `${BLUE}${"█".repeat(serverA)}${
      " ".repeat(barWidth - serverA)
    }${RESET} `;
...
async function main() {
  ...
  allStatusCodes.forEach((code) => {
    ...
    statusCodeRows.push([
      code,
      `${BLUE}${countA} (${
        formatNumber((countA / serverAResult.total) * 100, 1)
      }%)${RESET}`,
      `${GREEN}${countB} (${
        formatNumber((countB / serverBResult.total) * 100, 1)
      }%)${RESET}`,
    ]);
  });

  console.log(
    createTable(["Status Code", "Jetpath", "Elysia"], statusCodeRows),
  );
  ...
  const percentImprovement =
    ((Math.max(scoreA, scoreB) / Math.min(scoreA, scoreB)) - 1) * 100;

  let winnerBox;
  if (scoreDiff < 0.05) {
    winnerBox = createBox(
      `${YELLOW}${BOLD}⚖️  PERFORMANCE DIFFERENCE IS NEGLIGIBLE${RESET}\n` +
  ...
  if (serverAResult.failed > 0 || serverBResult.failed > 0) {
    recommendations.push(
      `• ${YELLOW}Investigate failed requests to improve reliability.${RESET}`,
    );
  }
  ...
  if (serverAResult.p95 > 1000 || serverBResult.p95 > 1000) {
    recommendations.push(
      `• ${YELLOW}High p95 response times detected. Consider optimizing slow paths in your code.${RESET}`,
    );
  }

  // Check for throughput
  const throughputDiff =
    Math.abs(serverAResult.throughput - serverBResult.throughput) /
    Math.max(serverAResult.throughput, serverBResult.throughput);
  if (throughputDiff > 0.3) {
    recommendations.push(
      `• ${YELLOW}Significant difference in throughput detected. The slower server may benefit from response compression or optimization.${RESET}`,
    );
  }
  ...
  const calcVariance = (result: BenchmarkResult) => {
    ...
    const variance = result.responseTimes.reduce(
  ...
  if (cvA > 0.5 || cvB > 0.5) {
    recommendations.push(
      `• ${YELLOW}High variance in response times detected. Consider investigating resource contention or GC pauses.${RESET}`,
    );
  }

  if (recommendations.length === 0) {
    recommendations.push(
      `• ${GREEN}Both servers appear to be performing well within expected parameters.${RESET}`,
    );
  }
  ...
  if (scoreA > scoreB) {
    console.log(
      `${BLUE}Jetpath${RESET} handled ${BOLD}${
        formatNumber(serverAResult.rps)
      }${RESET} requests/second with ${BOLD}${
        formatNumber(serverAResult.avgResponseTime)
      }ms${RESET} average response time`,
    );
    console.log(
      `${GREEN}Elysia${RESET} handled ${BOLD}${
        formatNumber(serverBResult.rps)
      }${RESET} requests/second with ${BOLD}${
        formatNumber(serverBResult.avgResponseTime)
      }ms${RESET} average response time`,
    );

    if (serverAResult.rps > serverBResult.rps) {
      console.log(
        `\n${BLUE}Jetpath${RESET} is ${BOLD}${
          formatNumber((serverAResult.rps / serverBResult.rps - 1) * 100)
        }%${RESET} faster in throughput`,
      );
    }
    if (serverBResult.avgResponseTime > serverAResult.avgResponseTime) {
      console.log(
        `${BLUE}Jetpath${RESET} is ${BOLD}${
          formatNumber(
            (serverBResult.avgResponseTime / serverAResult.avgResponseTime -
              1) * 100,
          )
        }%${RESET} faster in response time`,
      );
    }
  } else {
    console.log(
    ...
    if (serverBResult.rps > serverAResult.rps) {
      console.log(
        `\n${GREEN}Elysia${RESET} is ${BOLD}${
          formatNumber((serverBResult.rps / serverAResult.rps - 1) * 100)
        }%${RESET} faster in throughput`,
      );
    }
    if (serverAResult.avgResponseTime > serverBResult.avgResponseTime) {
      console.log(
        `${GREEN}Elysia${RESET} is ${BOLD}${
          formatNumber(
            (serverAResult.avgResponseTime / serverBResult.avgResponseTime -
              1) * 100,
          )
        }%${RESET} faster in response time`,
      );
